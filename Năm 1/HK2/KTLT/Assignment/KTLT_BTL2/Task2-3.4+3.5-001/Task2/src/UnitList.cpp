...
bool UnitList::checkSpecialNumber(int val){  // Có thể Sử dụng thuật toán vét cạn backTracking để tính toán 
    ...
    vector <int> base = {3,5,7};  // Xác định cơ số nó là một số nguyên tố lẻ nhỏ hơn 10
    for (int i: base){
...
//     Node *newNode = new Node (unit); // Khởi tạo node mới xử lý trên danh sách Deep Copy ko Copy Unit nữa 
//     if (! tail ) {
...
//     }
// }
void UnitList::insertAtEndRaw (Unit * unit){
    // Node *newNode = new Node (deepCopyUnit(unit)); // Khởi tạo node mới xử lý trên danh sách Deep Copy
    Node *newNode = new Node (unit); // Khởi tạo node mới xử lý trên danh sách Deep Copy ko Copy Unit nữa 
    if (! tail ) {
  // Lấy phần đuôi trỏ đến ngay phần tử mới insert at the end list 
...
void UnitList::insertAtEnd(Unit* unit){
    insertAtEndRaw(deepCopyUnit(unit));
}
...
// Unit* UnitList:: identifyUnit(Unit* unit) {
//     Unit* found;
//     if (isVehicleType(unit) && isContain((VehicleType)unit->checkValue())) {
//         found = identifyUnit((VehicleType)unit->checkValue());
//         return found;
//     }

//     if (isInfantryType(unit) && isContain((InfantryType)unit->checkValue())) {
//         found = identifyUnit((InfantryType)unit->checkValue());
//         return found;
//     }
...
Unit*  UnitList:: identifyUnit (InfantryType type){  // Check có phải là kiểu dữ liệu Infantry
    for (Node * curr = head; curr;curr = curr-> next){
        if (isInfantryType(curr-> unit) && curr -> unit -> checkValue() == (int)type ) return (curr -> unit);
    }
    return nullptr;
}
Unit* UnitList:: identifyUnit (VehicleType type){  // Check xem có phải là kiểu dữ liệu Vehicle 
    for (Node * curr = head; curr;curr= curr-> next){
        if (isVehicleType(curr-> unit) && curr -> unit -> checkValue() ==(int) type ) return(curr -> unit);
    }
    return nullptr;
}
bool UnitList::insert(Unit *unit)
{
    ...
    // Nếu là Unit đã có sắn trong danh sách thì chỉ cập nhật lại số lượng quantity 
        // Hàm kiểm tra sự tồn tại trong danh sách đã được kiểm thử 
    // if (isContain(unit)){
    ...
    // }
        // Hàm viết lại kỹ hơn 
     if (isContain(unit)){
        if (isVehicleType(unit)){
            Unit *identity = identifyUnit ((VehicleType)unit -> checkValue());
            if (identity) {
            updateQuantity(identity,unit);  
        }
        } else {
           Unit *identity = identifyUnit ((InfantryType)unit -> checkValue());
           if (identity) {
           updateQuantity(identity,unit);  
        }
        // Nếu con trỏ identity trả về giá trị thì đúng nếu là nullptr thì kết quả là sai
    ...
    // Nếu là vehicle chèn vào cuối list            // Câu lệnh đã được kiểm thử 
    ...
    if (isInfantryType(unit)){  // Kiểm tra xem đơn vị unit có đơn vị Infantry hay không
      // Nếu là lực lương bộ binh infantry chèn vào đầu list 
        Unit * newUnit = deepCopyUnit(unit);  // Can thiệp sâu vào danh sách bằng newUnit
        Node * newNode = new Node (newUnit);   // Khởi tạo một node mới bằng unit can thiệp mới 
        newNode -> next = head;
...
void UnitList ::updateQuantity(Unit* unitCurrent, Unit* addUnit){
        // Chèn thêm kiểu int mang tính chính xác tại vị trí này 
    if ((int)unitCurrent -> getWeight()< (int) addUnit-> getWeight()) {
        // Cập nhật lại weight và pos
        unitCurrent ->setWeight(addUnit-> getWeight());
        unitCurrent -> setPos(addUnit->getCurrentPosition());
    }
    unitCurrent -> setquantity((int)(unitCurrent -> getquantity()+ addUnit-> getquantity()));
  }
...
// Tạo toán tử operator = Phương pháp này rất cần thiết trong  phương pháp deepCopy tránh tình trạng Swallow copy rất cần thiết 
