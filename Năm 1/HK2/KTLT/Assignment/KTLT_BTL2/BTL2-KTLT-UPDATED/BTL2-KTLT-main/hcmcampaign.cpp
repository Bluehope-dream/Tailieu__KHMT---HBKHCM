...
// battleField );  - model để setup army
    ...
    LibertyUnitArray = new Unit*[liberUnits.size()];
    ARVNUnitArray = new Unit*[arvnUnits.size()];
    
    for (size_t i = 0; i < liberUnits.size(); ++i) {
        LibertyUnitArray[i] = liberUnits[i];
    }
    ...
    Army* arvnTemp = nullptr;
    Army* liberTemp = nullptr;
    initialArmy(liberTemp, liberUnits, LibertyUnitArray, false);
    initialArmy(arvnTemp, arvnUnits, ARVNUnitArray, true);
    // liberationArmy = static_cast<LiberationArmy*>(liberTemp);
    // ARVNArmy = static_cast<ARVN*>(arvnTemp);
}
...
// Xét các chỉ số quân sự khi đề cập đến EVENTCODE 
    ...
    // cout << "STEP 4 "<<endl;
    if (eventCode  >= 75){
    Army* enemyLiber = ARVNArmy;
    // cout << "STEP 4.1 IF "<<endl;
    ARVNArmy -> fight (ARVNArmy,false);       // Quân đôi Sài Gòn tấn công 
    // cout << "STEP 4.2 IF "<<endl;
    liberationArmy -> fight (enemyLiber,true);
    // cout << "STEP 4.3 IF "<<endl;
    liberationArmy -> fight (enemyLiber,false);    // Quân đội Giai phòng tân công 
    // cout << "STEP 4.4 IF "<<endl;
    ARVNArmy -> fight (ARVNArmy,true);   
    // cout << "STEP 4.5 IF "<<endl;

    } else {
   // Quân Gỉai Phóng trong thế tấn công 
   Army* enemyLiber = ARVNArmy;
//    cout << "STEP 4.1 ESLE "<<endl;
//    cout << liberationArmy->str()<<endl;
//    cout << "STEP 4.2 ESLE "<<endl;
    // liberationArmy-> fight (liberationArmy,false);
    liberationArmy-> fight (enemyLiber,false);
    
    // cout << "STEP 4.3 ESLE "<<endl;
   // Quân Sài Gòn trong thế phòng thủ 
    ARVNArmy -> fight (ARVNArmy,true);   
    // cout << "STEP 4.4 ESLE "<<endl;
...
void HCMCampaign::initialArmy(Army*& armyName, std::vector<Unit*>& unit, Unit**& unitArray, bool isARVN) {
    size_t total = unit.size();
    for (size_t i = 0; i < total; ++i) {
        unitArray[i] = unit[i];
    }
    if (isARVN) {
        // armyName = new ARVN(unitArray, total, "ARVN Army", this->battleField);
        this->ARVNArmy = new ARVN(unitArray, total, "ARVN Army", this->battleField);
        // this->ARVNArmy = armyName;
    } else {
        // armyName = new LiberationArmy(unitArray, total, "Liberation Army", this->battleField);
        this->liberationArmy = new LiberationArmy(unitArray, total, "Liberation Army", this->battleField);
    }
}

void Configuration::releaseVector(std::vector<Position*> vec) {
    for (Position* ptr : vec) {
        delete ptr; 
    }
}

void Configuration::releaseVector(std::vector<Unit*> vec) {
    for (Unit* ptr : vec) {
        delete ptr; 
    }
}