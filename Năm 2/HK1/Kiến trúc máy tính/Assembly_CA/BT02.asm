...
not_in_circle:
    ...
    # ==========================================
    # PHAN 1: XUAT RA MAN HINH (CONSOLE)
    # ==========================================
    ...
    li      $a2, 0
    syscall
    move    $s0, $v0         # Luu file descriptor

    # Kiem tra loi mo file
    bltz    $s0, end_program

    # 2. Ghi "So diem nam trong hinh tron: "
    move    $a0, $s0
    la      $a1, str1
    li      $a2, 29
    li      $v0, 15
    ...
    # --- KY THUAT GHI SO THUC (FLOAT) RA FILE ---
    # Xu ly phan nguyen
    ...
    # Nhan voi 100,000 de lay 5 so le
    l.s     $f14, f_100k
    mul.s   $f13, $f13, $f14 # $f13 = 14159.xxxx
    cvt.w.s $f13, $f13       # $t6 = 14159 (Int)
    mfc1    $t6, $f13
    abs     $t6, $t6         # Lay tri tuyet doi de tranh loi

    # --- FIX LOI MAT SO 0 O DAU (VD: 3.04159) ---
    # Kiem tra neu phan thap phan < 10000 thi thieu so 0
    li      $t7, 10000
    bge     $t6, $t7, write_decimal # Neu >= 10000 thi ghi binh thuong
    
    # Neu nho hon, can them so 0
    move    $a0, $s0
    ...
    # (Co the can check them < 1000, < 100... nhung voi PI random thi hiem khi xay ra)
